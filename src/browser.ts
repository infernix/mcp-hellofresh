...
export class HelloFreshBrowser {
    ...
    try {
      ...
      return deliveries.map((delivery) => ({
        ...
        meals: HelloFreshBrowser.asArray(delivery.recipes ?? delivery.meals).map((meal) => {
          const recipe = HelloFreshBrowser.recordValue((meal as JsonObject).recipe) ?? (meal as JsonObject);
          return {
            recipeId: HelloFreshBrowser.stringValue(recipe.id ?? (meal as JsonObject).recipeId),
            recipeName: HelloFreshBrowser.stringValue(recipe.name ?? (meal as JsonObject).recipeName),
            servings: HelloFreshBrowser.numberValue((meal as JsonObject).servings, 2),
          };
        }),
  ...
  async updatePreferences(
    ...
    if (preferences.vegetarian !== undefined) {
      await this.setCheckboxByLocator(
  ...
  async getPastOrders(limit = 10): Promise<Order[]> {
    ...
    try {
      ...
      return HelloFreshBrowser.asArray(data.items ?? data.orders)
        .slice(0, limit)
        .map((order) => {
          const record = order as JsonObject;
          return {
            ...
            meals: HelloFreshBrowser.asArray(record.recipes ?? record.meals).map((meal) => {
              const mealRecord = meal as JsonObject;
  ...
  private async scrapeDeliveryScheduleFromCurrentPage(): Promise<DeliveryInfo[]> {
    const page = this.page!;
  ...
  private async scrapePastOrdersFromCurrentPage(limit: number): Promise<Order[]> {
    const page = this.page!;
    await page.goto(`${this.baseUrl}/my-account/orders`, { waitUntil: "domcontentloaded" });
    await page.waitForLoadState("networkidle", { timeout: 20_000 }).catch(() => {});
    return page.evaluate((maxOrders: number) =>
      Array.from(document.querySelectorAll('[data-testid*="order"], [class*="OrderCard"], [class*="order-card"], article'))
        .slice(0, maxOrders)
        .map((el) => ({
  ...
  private async setCheckboxByLocator(locator: ReturnType<Page["locator"]>, value: boolean): Promise<void> {
    if (await locator.isVisible({ timeout: 3_000 }).catch(() => false)) {
      const isChecked = await locator.isChecked().catch(() => false);
      if (value !== isChecked) await locator.click();
    }
  }
