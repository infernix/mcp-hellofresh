...
const GetMenuForWeekSchema = z.object({
  week_id: z.string().describe("The week identifier (e.g. '2024-W01') for the delivery"),
});
...
const AddRecommendedExtrasSchema = z.object({
  week_id: z.string().describe("The week identifier (e.g. '2024-W01') for the delivery"),
});

const GetRecommendedExtrasSchema = z.object({
  week_id: z.string().describe("The week identifier (e.g. '2024-W01') for the delivery"),
  recipe_ids: z
    .array(z.string())
    .optional()
...
const SetMealExtrasSchema = z.object({
  week_id: z.string().describe("The week identifier (e.g. '2024-W01') for the delivery"),
  extras: z
    .array(
      z.object({
        addon_index: z.number().int().describe("The add-on index from get_recommended_extras"),
        meal_index: z.number().int().optional().describe("The selected meal index this add-on should attach to"),
        meal_recipe_id: z.string().optional().describe("Alternative to meal_index: selected meal recipe ID this add-on should attach to"),
        quantity: z.number().int().min(1).max(10).optional().describe("Quantity to request for this add-on"),
      })
    )
    .min(1)
...
const WeekPlanMealSchema = z.object({
  recipe_id: z.string().describe("Recipe ID to select"),
  servings: z
    .number()
    .int()
  ...
  extras: z
    .array(
      z.object({
        addon_index: z.number().int().describe("Add-on index attached to this meal"),
        quantity: z.number().int().min(1).max(10).optional().describe("Quantity to request for this add-on"),
      })
    )
    .optional()
...
const ApplyWeekPlanSchema = z.object({
  week_id: z.string().describe("The week identifier (e.g. '2024-W01') for the delivery"),
  meals: z.array(WeekPlanMealSchema).min(1).describe("Meals to select, each with optional meal-bound extras"),
});
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        if (this.readOnly && MUTATING_TOOL_NAMES.has(name)) {
          throw new Error(
            `Tool ${name} is disabled because HELLOFRESH_READ_ONLY is enabled.`
          );
        }
  ...
  private async handleTool(
    ...
    switch (name) {
      ...
      case "get_recommended_extras": {
        ...
        const result = await this.hellofresh.getRecommendedExtras(
          params.week_id,
          params.recipe_ids
        );
        return text({
      ...
      case "preview_meal_extras": {
        ...
        const result = await this.hellofresh.previewMealExtras(
          params.week_id,
      ...
      case "preview_select_meals": {
        ...
        const result = await this.hellofresh.previewSelectMeals(
          params.week_id,
      ...
      case "add_recommended_extras": {
        ...
        const result = await this.hellofresh.addRecommendedExtras(
          params.week_id
        );
        return text(result);
      }
      ...
      case "set_meal_extras": {
        ...
        const result = await this.hellofresh.setMealExtras(
          params.week_id,
          params.extras.map((extra) => ({
            addonIndex: extra.addon_index,
            mealIndex: extra.meal_index,
